...
public class PhysicsComponentConverter {
    ...
    /**
     * Converts {@link AbstractPhysicsComponent} to {@link PhysicsComponentDTO}.
     */
    public static PhysicsComponentDTO convert(AbstractPhysicsComponent component) {
        PhysicsComponentDTO componentDTO = new PhysicsComponentDTO();
        componentDTO.setPhysicsShape(component.getPhysicsShape());
        componentDTO.setPhysicsBodyType(component.getPhysicsBodyType());

        if (component instanceof RigidBodyPhysicsComponent) {
            componentDTO.setFriction(((RigidBodyPhysicsComponent) component).getFriction());
            componentDTO.setRestitution(((RigidBodyPhysicsComponent) component).getRestitution());
            componentDTO.setMass(((RigidBodyPhysicsComponent) component).getMass());
            componentDTO.setDisableDeactivation(((RigidBodyPhysicsComponent) component).isDisableDeactivation());
        }
